...
const Poll: React.FC<IProps> = ({ waku }) => {
    ...
    const [voteCounts, setVoteCounts] = useState<number[]>(new Array(questions[0].answers.length).fill(0));
                ...
                <div className="bg-white p-6 rounded-lg shadow-lg">
                            ...
                            <div key={index}>
                                <button
                                    className={`w-full text-lg py-2 px-4 rounded-lg transition-colors duration-300 ${
                                        vote === null ? 'bg-blue-500 hover:bg-blue-600 text-white' : 'bg-gray-300 text-gray-500 cursor-not-allowed'
                                    }`}
                                ...
                                <p className="text-sm text-gray-600 mt-1">Votes: {voteCounts[index]}</p>
                            </div>
                        ))}
                    </div>
                    ...
                    {vote !== null && (
                        <div className="mt-6 p-4 rounded-lg bg-green-100 text-green-800">
                            <p className="font-medium">You voted for: {question.answers[vote]}</p>
                        </div>
                    )}
                </div>
            </div>
